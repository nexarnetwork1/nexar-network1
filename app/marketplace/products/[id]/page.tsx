...
export default async function PublicMarketplaceProductPage({ params }: Props) {
  const { id } = await params;
  const product = await getMarketplaceProductWithDetails(id);
      ...
      <div className="relative min-h-[calc(100vh-var(--nxr-header-offset))]">
        ...
        <Container as="div" className="relative py-10 sm:py-12">
          <ProductDetailClient product={product}>
            ...
            <Link href="/marketplace/browse" className="text-sm text-muted hover:text-gold">
              ← Back to marketplace
            </Link>
              ...
              <div>
                ...
                {ratingSummary.count > 0 && (
                  <p className="mt-2 text-sm text-amber-400">
                    ★ {ratingSummary.avg} · {ratingSummary.count} review
            ...
            <ProductRecommendations
              ...
              products={recommended.filter((p) => p.id !== product.id).slice(0, 4)}
              id="rec-heading"
              productBasePath="/marketplace/products"
            />
          </ProductDetailClient>
